...
function toProcessoInfo(path: string, requestedSlug: string): ProcessoInfo {
  const fileName = path.split('/').pop() || path
  const pathParts = path.split('/')

  return {
    slug: normalizeIncomingSlug(path.replace(/\.bpmn$/i, '')),
    nome: fileName.replace(/\.bpmn$/i, ''),
    file: path,
    arquivo: path,
    categoria: pathParts.length > 1 ? pathParts[0] : 'Raiz',
    bpmnUrl: `/api/bpmn/${encodeURIComponent(requestedSlug)}`,
    descriptionsUrl: '/api/descriptions',
    contentUrl: `/api/content/${encodeURIComponent(requestedSlug)}`,
  }
}
...
async function findProcesso(slug: string): Promise<{ atual: ProcessoInfo; outros: ProcessoInfo[] } | null> {
  ...
  const githubMatch = githubFiles.find((file) => file.slug === normalizedSlug)
  ...
  if (githubMatch) {
    ...
    const outros = githubFiles
      ...
      .map((item) => toProcessoInfo(item.path, item.slug))

    return { atual: toProcessoInfo(githubMatch.path, slug), outros }
  }

  const localFiles = listLocalBpmnFiles()
  const localMatch = localFiles.find((file) => file.slug === normalizedSlug)
  ...
  return { atual: toProcessoInfo(localMatch.path, slug), outros }
}
