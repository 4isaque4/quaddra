...
async function findProcesso(slug: string, githubRepo: string = GITHUB_REPO_QUADDRA): Promise<{ atual: ProcessoInfo, outros: ProcessoInfo[] } | null> {
  ...
  if (!match) {
    ...
    if (octokit) {
      console.log('[PROCESSO PAGE] Tentando buscar no GitHub...')
      try {
...
export default async function ProcessoPage({ params }: { params: Promise<{ slug: string }> | { slug: string } }) {
  ...
  // Usar repositório QUADDRA por padrão (página padrão)
  const githubRepo = GITHUB_REPO_QUADDRA
  
  console.log('[PROCESSO PAGE] Parâmetros recebidos:', resolvedParams)
  console.log('[PROCESSO PAGE] Slug:', slug)
  console.log('[PROCESSO PAGE] Repositório GitHub:', githubRepo)
  
  const resultado = await findProcesso(slug, githubRepo)
